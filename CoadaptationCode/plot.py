...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Specific episode (checkpoint) to generate a standalone trajectory plot for.",
    )
    parser.add_argument(
        "--start-from",
        type=int,
        default=None,
        help="Filter all timeline plots to only include data starting from this episode.",
    )
...
def plot_single_checkpoint(rewards: pd.DataFrame, episode: int, output_dir: Path) -> Path | None:
    """Plot a detailed XY trajectory for a single specific episode/checkpoint."""
    ep_frame = rewards[rewards["Episode"] == episode].sort_values("Timestep")
    if ep_frame.empty:
        print(f"Warning: No data found for checkpoint episode {episode}")
        return None
    ...
    terrain = ep_frame["Terrain"].iloc[0] if "Terrain" in ep_frame.columns else "Unknown"
    color = color_for_terrain(terrain)
    
    ax.plot(x_relative, y_relative, lw=2.5, color=color, label=f"Episode {episode}")
    ax.scatter(x_relative.iloc[0], y_relative.iloc[0], color="#222222", s=60, zorder=5, label="Start")
    ax.scatter(x_relative.iloc[-1], y_relative.iloc[-1], color=color, marker="x", s=80, zorder=5, label="End")
    
    ax.set_title(f"Checkpoint Trajectory: Episode {episode} | Terrain: {terrain.replace('_', ' ')}")
    ax.set_aspect("equal", adjustable="datalim")
    ax.legend(frameon=False)
    ax.set_ylabel("Y displacement from start")
    finish_axis(ax, xlabel="X displacement from start")
    
    fig.tight_layout()
    path = output_dir / f"checkpoint_{episode}_trajectory.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    return path
...
def main() -> None:
    ...
    if args.start_from is not None:
        cleaned_rewards = cleaned_rewards[cleaned_rewards["Episode"] >= args.start_from]
        cleaned_losses = cleaned_losses[cleaned_losses["Episode"] >= args.start_from]
    ...
    if args.checkpoint_episode is not None:
        cp_plot = plot_single_checkpoint(cleaned_rewards, args.checkpoint_episode, output_dir)
        plots.append(cp_plot)
